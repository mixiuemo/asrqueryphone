...
import { Table, Button, Space, Modal, Form, Input, message, Upload, Radio, Checkbox, Tag  } from 'antd';
import { ExclamationCircleOutlined, UploadOutlined, PlayCircleOutlined, DownloadOutlined } from '@ant-design/icons';
...
const VoicePrintList = () => {
    ...
    const fetchData = async (searchQuery = '') => {
        setLoading(true);
        try {
            const result = await getEmbedding(searchQuery);
            setData(result);
            // 生成从 1 开始的序号
    ...
    // 新增测试声纹方法
    const onTestVoice = () => {
        setTestVoiceTitle('语音提问');
        setTestVoiceVisible(true);
        setWhoIs('');
    };
    const handleEdit = (record) => {
        setPlayName(record.uploadName)
        ...
        setIsModalVisible(true);
        setFileName(record.fileName || record.file);
        setAudioUrl(record.fileUrl || record.file);
        setIsFileUploaded(true);
        setSelectedOption('upload');
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingService) {
                await updateEmbedding(editingService.id, values);
                message.success('记录更新成功');
            } else {
                await createEmbedding(values);
    ...
    const downloadTemplate = () => {
        ...
        link.href = `${API_BASE_URL}/embedding/template`;
        link.download = 'embedding_template.xlsx';
        ...
        confirm({
          ...
          onOk: async () => {
            try {
              ...
              await Promise.all(ids.map(id => deleteEmbedding(id)));
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'userType',
            render: (text, record) => (
                <Tag style={{fontSize:14}} color={record.userType === 1 ? '#7DC67D' : (record.userType === 2 ? '#33BCBA' : '#E17572')}>
                    {record.userType === 1 ? '普通用户' : (record.userType === 2 ? '话务员' : '重要用户')}
                </Tag>
            ),
        },
        {
    ...
    return (
        <div>
            <Space style={{ marginBottom: 16 }}>
                ...
                <Button type="primary" style={{ marginLeft: 40 }} onClick={onTestVoice}>
                    测试声纹
                </Button>
                <Upload
                    name="file"
                    accept=".xls,.xlsx"
                    action={`${API_BASE_URL}/embedding/importvoice`}
                    headers={{
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }}
                    beforeUpload={(file) => {
            ...
            <Modal
                title={editingService ? '编辑声纹' : '添加声纹'}
                open={isModalVisible}
                onOk={handleSubmit}
                onCancel={() => setIsModalVisible(false)}
                width={600}
            >
                <Form form={form} layout="vertical" initialValues={{ option: 'upload',userType : userType }}>
                    <Form.Item
                        name="speakerName"
                        label="用户名"
            ...
            {/* 新增测试声纹的Modal */}
            <Modal
                title={testVoiceTitle}
                open={testVoiceVisible}
                maskClosable={false}
                width={800}
                destroyOnClose
                ...
                footer={[
                    ...
                    <Button
                        key="submit"
                        type="primary"
                        onClick={() => setTestVoiceVisible(false)}
                    >
                        完成
                    </Button>
                ]}
