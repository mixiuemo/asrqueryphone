...
const VoiceRegister = ({ switchToVoiceList }) => {
  ...
  const initWaveform = (filePath) => {
    ...
    // 加载音频文件
    wavesurfer.load(`${API_BASE_URL}/embedding/file?path=${encodeURIComponent(filePath)}`);
    ...
    // 修复3: 统一绑定时间更新事件
    //   timeUpdateHandlerRef.current = (currentTime) => {
    //     if (isPlayingRegion && currentRegionRef.current) {
    //       const region = currentRegionRef.current;
    //       const regionDuration = region.end - region.start;
    //       const progress = ((currentTime - region.start) / regionDuration) * 100;
    //       setPlaybackProgress(progress);
    //     } else if (isPlaying) {
    //       setPlaybackProgress((currentTime / dur) * 100);
    //     }
    //   };
